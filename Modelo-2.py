...
class Peaton(ap.Agent):
    ...
    def execute(self):
        if self.next_step >= len(self.route):
            if self.respect == 1:
                self.model.disobedient.append(self.step)
            else:
                self.model.obedient.append(self.step)
            ...
            if len(agents) != 0:
                ...
                if dir == DOWN:
                    ...
                    if len(agents) != 0:
                        agents = [agent for agent in self.model.environment.get_agents_in_dir(self,[UP],Car)]
                        agents.append(self.model.environment.get_agents_in_dir(self,[UP],Peaton))
                        if len(agents) == 0:
                            self.model.environment.move_by(self,UP)
                            self.next_step -= 1
                            self.step += 1
                        elif len([agent for agent in self.model.environment.get_agents_in_dir(self,[RIGHT],Car) or agents for agent in self.model.environment.get_agents_in_dir(self,[RIGHT],Peaton)]) == 0:
                            self.model.environment.move_by(self,RIGHT)
                            self.step += 1
                    else:
                    ...
                    if len(agents) != 0:
                        ...
                        if len(agents) == 0:
                            self.model.environment.move_by(self,UP)
                            self.step += 1
                        elif len([agent for agent in self.model.environment.get_agents_in_dir(self,[LEFT],Car) or agents for agent in self.model.environment.get_agents_in_dir(self,[LEFT],Peaton)]) == 0:
                            self.model.environment.move_by(self,LEFT)
                            self.step += 1
                            self.next_step -= 1   
                    else:
                ...
                agents = [agent for agent in self.model.environment.agents[next_pos] if isinstance(agent, Car)]
                if len(agents) != 0:
                    self.step += 1
                else:
...
promedio_obedientes = np.mean(cityModel.obedient)
promedio_desobedientes = np.mean(cityModel.disobedient)

# Crear la gráfica
plt.figure(figsize=(10, 6))  # Tamaño de la gráfica

# Graficar las líneas para cada tipo de agente
plt.plot(cityModel.obedient, label='Agentes Obedientes', marker='o', linestyle='-', color='blue')
plt.plot(cityModel.disobedient, label='Agentes Desobedientes', marker='s', linestyle='--', color='red')

# Añadir líneas horizontales para los promedios
plt.axhline(promedio_obedientes, color='blue', linestyle=':', label=f'Promedio Obedientes: {promedio_obedientes:.2f}')
plt.axhline(promedio_desobedientes, color='red', linestyle=':', label=f'Promedio Desobedientes: {promedio_desobedientes:.2f}')

# Añadir etiquetas y título
plt.xlabel('Número de Agente')
plt.ylabel('Pasos para llegar al destino')
plt.title('Comparación de Agentes Obedientes vs Desobedientes')
plt.legend()
